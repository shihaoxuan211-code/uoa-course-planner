...
import type { EasyAIndex } from "@/lib/gradeOutlook";
...
export function GradeOutlook({ course, review }: GradeOutlookProps) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-card">
      <h2 className="text-xl font-bold text-ink">{t.gradeOutlook.heading}</h2>

      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg bg-slate-50 p-3 text-center">
          <p className="text-xs font-semibold uppercase tracking-normal text-slate-500">{t.gradeOutlook.easyAIndex}</p>
          <p className="mt-2"><EasyABadge level={outlook.easyAIndex} /></p>
        </div>
        <div className="rounded-lg bg-slate-50 p-3 text-center">
          <p className="text-xs font-semibold uppercase tracking-normal text-slate-500">{t.gradeOutlook.aRangePotential}</p>
          <p className="mt-2"><LevelBadge level={outlook.aRangePotential} type="potential" /></p>
        </div>
        <div className="rounded-lg bg-slate-50 p-3 text-center">
          <p className="text-xs font-semibold uppercase tracking-normal text-slate-500">{t.gradeOutlook.workloadLevel}</p>
          <p className="mt-2"><LevelBadge level={outlook.workloadLevel} type="workload" /></p>
        </div>
        <div className="rounded-lg bg-slate-50 p-3 text-center">
          <p className="text-xs font-semibold uppercase tracking-normal text-slate-500">{t.gradeOutlook.riskLevel}</p>
      ...
      {outlook.reasons.length > 0 && (
        <div className="mt-4 rounded-lg bg-slate-50 p-4">
          <p className="text-xs font-semibold uppercase tracking-normal text-slate-500">{t.gradeOutlook.why}</p>
      ...
      <p className="mt-4 text-xs text-slate-400">
        {t.gradeOutlook.disclaimer} {outlook.disclaimer}
      </p>
    </section>
  );
}
